...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(1333, 800), keep_ratio=True, with_mask=True),
...
model = dict(
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            ...
            roi_feat_size=7,
            num_classes=11,     # changed class number
            bbox_coder=dict(
                type='DeltaXYWHBBoxCoder',
                target_means=[0., 0., 0., 0.],
...
# optimizer
optimizer = dict(type='SGD', lr=0.0025, momentum=0.9, weight_decay=0.0001)      # modified lr
...
lr_config = dict(
    ...
    warmup_ratio=0.001,
    step=[8, 11])
runner = dict(type='EpochBasedRunner', max_epochs=1)       # modified max_epoch
